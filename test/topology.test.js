...
test("buildTopology creates root-only sqlite authority topology", () => {
  ...
  assert.equal(topology.sources.punkin.ref, "w/router");
...
test("buildBootstrapBundle emits git-based root bootstrap script", () => {
  ...
  assert.match(bundle.scripts.root, /git rev-parse --verify -q 'refs\/tags\/w\/router' >/);
  assert.match(bundle.scripts.root, /git -c advice\.detachedHead=false checkout --detach 'refs\/tags\/w\/router'/);
  ...
  assert.match(bundle.scripts.root, /PUNKIN_RELEASE_TAG='w\/router'/);
